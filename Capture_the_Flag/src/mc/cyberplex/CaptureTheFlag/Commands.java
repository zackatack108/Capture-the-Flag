...
import mc.cyberplex.CaptureTheFlag.kits.KitShop;
...
public class Commands implements CommandExecutor{
	...
	public boolean onCommand(CommandSender sender, Command cmd, String string, String[] args){
		...
		if(sender instanceof Player){
			...
			//check if the base command is for Capture the flag
			if(cmd.getName().equalsIgnoreCase("ctf")){
				...
				case "minplayers":					
					minPlayersCmd(player, args);
					break;

				case "maxplayers":					
					maxPlayersCmd(player, args);
					break;
	...
	private void minPlayersCmd(Player player, String[] args) {
	...
	private void maxPlayersCmd(Player player, String[] args) {
		...
		if(player.hasPermission("ctf.help")){
			...
			if(args.length == 2) {
				...
				if(Integer.parseInt(args[1]) == 2) {
					...
					player.sendMessage(ChatColor.STRIKETHROUGH + "-----------------------------------------------------");
					player.sendMessage(ChatColor.DARK_AQUA +  "Capture the Flag Commands" + ChatColor.GREEN + " | pg 2 of 3");
					player.sendMessage(ChatColor.STRIKETHROUGH + "-----------------------------------------------------");
					...
					player.sendMessage(ChatColor.YELLOW + "/ctf pos1 [Arena]: " + ChatColor.GRAY + "Add position 1 for an arena area");
					player.sendMessage(ChatColor.YELLOW + "/ctf pos2 [Arena]: " + ChatColor.GRAY + "Add position 2 for an arena area");
					...
					player.sendMessage(ChatColor.STRIKETHROUGH + "-----------------------------------------------------");
					player.sendMessage(ChatColor.DARK_AQUA +  "Capture the Flag Commands" + ChatColor.GREEN + " | pg 3 of 3");
					player.sendMessage(ChatColor.STRIKETHROUGH + "-----------------------------------------------------");					
					...
					player.sendMessage(ChatColor.STRIKETHROUGH + "-----------------------------------------------------");
					player.sendMessage(ChatColor.DARK_AQUA +  "Capture the Flag Commands" + ChatColor.GREEN + " | pg 1 of 3");
					player.sendMessage(ChatColor.STRIKETHROUGH + "-----------------------------------------------------");
					...
					player.sendMessage(ChatColor.YELLOW + "/ctf kits: " + ChatColor.GRAY + "Opens the Capture the Flag kits");
	...
	private void kitsCmd(Player player) {

		boolean inGame = false, gameRunning = false;
		...
		if(inGame == true && gameRunning == true) {
			...
			KitShop shop = new KitShop();
			shop.createCTFShop(player);
		}
			...
			if(player.hasPermission("ctf.remove.spawn")){
				...
				if(main.getConfig().contains("Arenas." + args[2].toLowerCase())){
					...
					if(args.length == 4){
						...
						main.getConfig().set("Arenas." + args[2].toLowerCase() + "." + team + ".spawn", null);
						main.saveConfig();
			...
			if(player.hasPermission("ctf.remove.flag")){
				...
				if(main.getConfig().contains("Arenas." + args[2].toLowerCase())){
					...
					if(args.length == 4){
						...
						main.getConfig().set("Arenas." + args[2].toLowerCase() + "." + team + ".flag", null);
						main.saveConfig();
	...
	private void pos1Cmd(Player player, String[] args) {
		...
		//check if the player has permission to set the first position of an arena
		if(player.hasPermission("ctf.pos1")) {

			//check if the command has the position and the arena name
			if(args.length == 2) {
				...
				if(main.getConfig().contains("Arenas." + arenaName)) {
					...
					ctfData.setPos1(arenaName, player);
	...
	private void pos2Cmd(Player player, String[] args) {
		...
		//check if the player has permission to set the second position of an arena
		if(player.hasPermission("ctf.pos2")) {
			...
			if(args.length == 2) {
				...
				if(main.getConfig().contains("Arenas." + arenaName)) {
					CTFData ctfData = new CTFData();
					ctfData.setPos2(arenaName, player);
