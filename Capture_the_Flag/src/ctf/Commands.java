		...
		if(sender instanceof Player){
			...
			if(cmd.getName().equalsIgnoreCase("ctf")){
					...
					if(player.hasPermission("ctf.minplayers")){
						...
						if(args.length == 3){
							...
							if(config.contains("Arenas." + args[1].toLowerCase())){
								...
								config.set("Arenas." + args[1].toLowerCase() + ".Min", minNum);
								main.saveConfig();
					...
					if(player.hasPermission("ctf.maxplayers")){
						...
						if(args.length == 3){
							...
							if(config.contains("Arenas." + args[1].toLowerCase())){
								...
								config.set("Arenas." + args[1].toLowerCase() + ".Max", maxNum);
								main.saveConfig();
				...
				case "getarena":
					...
					Bukkit.broadcastMessage(Bukkit.getPlayer(UUID.fromString(data.getArena(arenaNum).getBlueTeam(0))).getName());
